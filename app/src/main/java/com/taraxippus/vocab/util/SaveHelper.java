...
public class SaveHelper
{
	...
	public SaveHelper(MainActivity main)
	{
		this.main = main;
	}
	...
	public void importVocabulary(String line, ImportType importType, VocabularyType type, boolean learned)
	{
		...
		if (!line.contains("【") || !line.contains("】"))
		{
			...
			if (line.contains("(") && line.contains(")"))
			{
				...
				v = new Vocabulary(type, StringHelper.trim(kanji), new String[0], meaning, StringHelper.trim(line.substring(line.indexOf("(") + 1, line.indexOf(")"))), "");
			}
			else
			{
				...
				v = new Vocabulary(type, StringHelper.trim(kanji), new String[0], meaning, "", "");
			}
		}
		else
		{
			...
			{		
				...
				v = new Vocabulary(type, StringHelper.trim(line.substring(0, line.indexOf("【"))), reading, meaning, StringHelper.trim(line.substring(line.indexOf("(") + 1, line.indexOf(")"))), "");
			}
			else
			{
				reading = line.substring(line.indexOf("【") + 1, line.indexOf("】")).split("/|／");
				...
				v = new Vocabulary(type, StringHelper.trim(line.substring(0, line.indexOf("【"))), reading, meaning, "", "");
			}
		}
	...
	public boolean load(FileInputStream stream)
	{
		...
		try 
		{
			...
			{
				DialogHelper.createDialog(main, "Corrupt save file", "The save file is corrupt. :(");
			}
			else
			{
				...
				while (buffer.hasRemaining())
				{
					...
					if (v != null)
					{
						...
						//main.dbHelper.updateVocabulary(v);
					}
				}
			}
		...
		{
			DialogHelper.createDialog(main, "Welcome!", "You can add new vocabularies from the \"Add new vocabulary\" tap in the navigation drawer. You can add them to your learned vocabularies by using the menus in the home tab. Learned vocabularies will appear in the quiz. A handwriting keyboard is recommended to enter kanji.");
		}
		catch (Exception e)
		{
			e.printStackTrace();
			...
			DialogHelper.createDialog(main, "Load", "Something went wrong while trying to load :(\nSaving will be disabled to prevent overriding data\n\n" + e.toString());
		}
	...
	public Vocabulary loadVocabulary(ByteBuffer buffer, int version)
	{
		if (version >= 1)
		{
			...
			Vocabulary v = new Vocabulary(type, kanji, reading, meaning, additionalInfo, notes);
